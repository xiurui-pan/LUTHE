...
// localparam
// ============================================================================================== --
  localparam real CLK_HALF_PERIOD = 2.5;
...
// ============================================================================================== --
  ...
  logic[TIMESTAMP_W-1:0]first_issue_time;
  logic[TIMESTAMP_W-1:0]last_retire_time;

  integer trace_event_counter = 0; 
  ...
  always_ff @(posedge clk) begin
    ...
    if (s_rst_n && trace_wr_en) begin
      //    isc_query_cmd_e 定义: NONE=0, RDUNLOCK=1, RETIRE=2, REFILL=3, ISSUE=4
      // $display("[TRACE EVENT #%0d] timestamp=%0d, cmd=%0d (%s), insn=0x%h", 
      //          trace_event_counter, 
      ...
      if (trace_data.cmd == ISSUE) begin
        if (trace_data.timestamp < first_issue_time) begin
          first_issue_time <= trace_data.timestamp;
        end        
      end

      else if (trace_data.cmd == RETIRE) begin
        if (trace_data.timestamp > last_retire_time) begin
          last_retire_time <= trace_data.timestamp;
        end
      end
...
// ============================================================================================== --
  ...
  if (!error) begin
    logic [TIMESTAMP_W-1:0] iop_total_cycles;
    iop_total_cycles = last_retire_time - first_issue_time;

    $display("DEBUG: Total trace events captured: %0d", trace_event_counter);
    if (trace_event_counter == 0) begin
        $display("FATAL_ERROR: No trace events were generated by the DUT. Instruction processing likely failed or stalled.");
    end

    $display("\n#####################################################################");
    $display("##                                                                 ##");
    $display("##               IOP EXECUTION SUMMARY                             ##");
    $display("##                                                                 ##");
    $display("#####################################################################");
    
    $display(">> Simulation clock frequence: %0f MHz", 1000.0/(CLK_HALF_PERIOD*2));
    $display(">> First DOP Issued at: %0d", first_issue_time);
    $display(">> Last DOP Retired at: %0d", last_retire_time);
    $display(">> TOTAL IOP CYCLE COUNT: %0d cycles", iop_total_cycles);
    $display(">> TOTAL time eplased: %0d us.", iop_total_cycles * CLK_HALF_PERIOD*2 / 1000);
    $display("#####################################################################\n");
  end
  end_of_test = 1'b1;
end
