...
###################################################################################################
# This script deals with the testbench run for WoP-PBS premodswitch engine.
...
while getopts "hzg:W:0:2:i:j:k:q:" opt; do
  case "$opt" in
    h)
      usage
      exit 0
...
if [ $GEN_STIMULI -eq 1 ] ; then
  ...
  echo "INFO> N=${N}, GLWE_K=${GLWE_K}, PBS_L=${PBS_L} LWE_K=$LWE_K MOD_Q=$MOD_Q MOD_Q_W=$MOD_Q_W N_LVL0=$N_LVL0 N_LVL2=$N_LVL2"
