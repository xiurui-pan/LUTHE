...
# premod_in  = flags[5] = 0x20 (backend sends preModSwitch() int32 payload; GPU skips normalize_pre_modswitch)
FLAGS="${FLAGS:-96}" # default: 0x40 | 0x20
...
WORD_BYTES="${WORD_BYTES:-8}"          # torus64
PREMOD_WORD_BYTES="${PREMOD_WORD_BYTES:-4}"  # int32
...
OUT_DIR="${OUT_DIR:-/tmp/csd_gpu_nvmevirt_cb_step4only_premod_${RUN_ID}}"
...
cfg "words" "tlwe_words=$TLWE_WORDS step4_words=$STEP4_WORDS glwe_words=$GLWE_WORDS word_bytes=$WORD_BYTES premod_word_bytes=$PREMOD_WORD_BYTES"
cfg "cb_msg" "$CB_MSG"
...
if [ "$PREMOD_WORD_BYTES" != "4" ]; then
  echo "[err] This script currently requires PREMOD_WORD_BYTES=4 (int32), got PREMOD_WORD_BYTES=$PREMOD_WORD_BYTES" >&2
  exit 1
fi
...
PREMOD_FILE="$OUT_DIR/cb_input_lvl0_msg${CB_MSG}.premod_i32.bin"
STEP4_CPU="$OUT_DIR/cb_step4_cpu.bin"
GOLDEN="$OUT_DIR/cb_step5_golden.bin"

echo "[3/4] Generate deterministic CB TLWE + golden (step5 on CPU) + premod(i32)"
CPU_REF_DUMP_PREMOD="$PREMOD_FILE" \
  "$CPU_RUNNER" \
    --mode 2 \
...
echo "[gen] sha256(premod_i32)=$(sha256sum "$PREMOD_FILE" | awk '{print $1}')"
...
if [ "$NO_SUDO" = "1" ]; then
  echo "[4/4] Run user-mode smoke (NO_SUDO=1): host-preMod(i32) -> GPU step4_only -> step5(cpu_reference_runner --privks-step4) -> cmp golden"
  if [ "$MODE" != "2" ]; then
  ...
  export GPU_SMOKE_WORD_BYTES="$PREMOD_WORD_BYTES"
  export GPU_SMOKE_GLWE_WORDS="$STEP4_WORDS"
  export GPU_SMOKE_GLWE_WORD_BYTES="$WORD_BYTES"
  export GPU_SMOKE_FLAGS="$FLAGS"

  STEP4_GPU="$OUT_DIR/cb_step4_gpu.bin"
  OUT_GLWE="$OUT_DIR/cb_step5_out.bin"

  "$SMOKE_BIN" CB "$PREMOD_FILE" "$STEP4_GPU" >"$OUT_DIR/smoke_cb_premod.log" 2>&1
...
echo "[4/4] Run nvmevirt e2e (needs sudo): 0xC0 -> backend (preMod + GPU step4_only + backend privks-step4, impl=$CSD_PRIVKS_IMPL) -> /dev/mem"
OUT_GLWE="$OUT_DIR/cb_step5_out.bin"
...
echo
echo "[PASS] CB step4_only + premod split e2e OK. Artifacts saved under: $OUT_DIR"
